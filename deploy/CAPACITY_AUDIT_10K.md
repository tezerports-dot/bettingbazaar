...
| token-revocation check | `auth.middleware.js` blacklist lookup | **1 PG read / request** |
| funding read + debit | wallet authority | PG, row-locked, in one transaction |
| bet insert | `bet.routes.js` | PG, same transaction as the debit |
| cycle open-gate + pool | `bet.routes.js` | PG — `FOR SHARE` on the cycle row; real pools **derived from `bets`**, never stored on that row |
| transaction log | `bet.routes.js` | PG ledger row, same transaction |
| broadcast | `bet.routes.js:419-422` SSE + `io.emit` | Redis pub + N sockets |
| rate limit | `security.js:170` `betLimiter` | Redis |

So one bet is **one PostgreSQL transaction** — the debit, the bet row and the ledger rows
commit together or not at all — plus **2 Redis ops** (limit + adapter publish) and 1
broadcast. That is the structural change worth noting for capacity: the old shape spread a
bet across two stores with a fire-and-forget mirror, so a bet could be half-written; the
current shape cannot, and its cost is one round trip holding one row lock.

**The exact op count per request is now UNKNOWN — requires re-measurement.** The counts
this section used to carry were read off two-store code that no longer exists, and this
document does not invent benchmarks. `loadtest/bet-contention.js` is the measurement.
...
5. Pool aggregation over `bets` for derived pools (`cyclePool.service.js`) — once/refresh.
...
**Top memory-heavy:** Argon2 (19 MiB/concurrent hash, §2); the database-dump backup
buffer (`backup.service.js`); worker-thread pool. Everything else is small.
...
What the code *does* tell us: the `api` process is **I/O-bound** (PostgreSQL/Redis awaits),
...
## 5. The datastore tier

**There is one: PostgreSQL (§6).** This section previously sized a second store that
carried the money path while PostgreSQL ran as a mirror. It is deleted — see `CLAUDE.md`.

Two findings from it survive because they are about the *workload*, not the engine, and
both are now enforced in the schema rather than mitigated by a flag:

- **The cycle row is the contention point.** Every real bet touches its cycle. Under the
  old shape each bet did an in-place increment of pool counters on that document, and
  same-document increments serialize. The current schema **does not store real pool
  totals on the `cycles` row at all** — a bet holds `FOR SHARE` on it, so a bet that also
  updated it would deadlock (40P01) against another bet doing the same. Real pools are
  **derived from `bets`**; only phantom figures are stored. The contention that needed a
  feature flag to avoid is now absent by construction.
- **Settlement is rare and batched** — one pass per cycle, ~**49 cycles/day** (48×30-min
  + 1 full-day; cycle length is `SystemConfig.cycleDurationMinutes` and must divide 60,
  `cycleGenerator.service.js`). Size for the bet rate, not the settlement rate.

**Sizing consequence: one tier, not two.** The RAM and vCPU that the second store's
section budgeted do not transfer one-for-one — PostgreSQL now carries the domains it
used to hold, so **the tier needs re-measuring** (§14) before hardware is bought. This
document does not invent that number.
...
- **Writes:** wallet ledger (double-entry, append-only, conserve-to-zero triggers), bet
  lifecycle, settlements, orders, KYC — **authoritative, and the only copy**. Plus every
  other domain: identity, configuration, CMS, engagement.
- **`BIGINT` reads back as a string** from node-postgres. Uncast, `'900' >= 1000` is true.
  Cast at the boundary — this is a correctness note, but it is here because a balance
  comparison that is silently wrong does not show up as a capacity symptom.
...
**Can Redis share a VPS with PostgreSQL at 10k DAU?** **Yes** — both have a small footprint
at this scale. Caveat: Redis is **realtime-critical** (it is the Socket.IO fan-out bus
*and* the leader lock *and* the rate-limit store), so co-locating it with the database
means a database spike can degrade all three. Acceptable at 10k; **give Redis its own instance by
...
|---|---|---|---|---|---|
| PG `bets` (+idx) | ~250–350 B | ~13–18 MB | ~1.2–1.6 GB | ~2.4–3.2 GB | ~5–6.5 GB |
| PG `transactions` | ~250 B | ~13 MB | ~1.2 GB | ~2.3 GB | ~4.6 GB |
| PG ledger (2–4 rows/bet) | ~200 B | ~20–40 MB | ~2–4 GB | ~4–7 GB | ~8–14 GB |
| PG `cycles` | tiny | 49 rows | negligible | | |
| Redis | working set | flat | ~0.5 GB | | |
| Logs | **UNKNOWN** | — | — | — | — |

So **order 18–25 GB of database growth in year one** at the assumed volume — comfortably
inside a 100–200 GB NVMe. The per-row figures are the same data in a narrower
representation (fixed columns and integer paise rather than repeated keys per document),
so treat the total as **the same order, slightly lower, and UNKNOWN in detail until
measured** — the row widths above are estimates from the schema, not observations.
Re-scale linearly with your real bets/day. Backups: `pg_dump → gzip → S3`
(`services/backup.service.js`) **plus WAL archiving off-box** — same-disk is not a backup,
and without WAL there is no point-in-time recovery, which is the one that matters for
money.
...
| Tier | What | Why |
|---|---|---|
| **A — required for function** | 1 VPS: Node (3 roles) + PostgreSQL + Redis + S3/MinIO + TLS | The boot gate refuses to start without all of these (`VPS_UBUNTU_SETUP.md:20-45`); money txns need the RS; login needs TLS |
...
- **1 × 8 vCPU / 8 GB / 200 GB NVMe** — everything (3 PM2 roles + PostgreSQL + Redis +
  MinIO), the `VPS_UBUNTU_SETUP.md` single-VPS build; Cloudflare Tunnel for edge.
- **~RM 183 ≈ ₹3,500/mo.**
- **Capacity:** 10k DAU at ≤~1,000 concurrent. (Derived pools are no longer a flag —
  they are how the schema works, §5.)
- **Bottleneck:** one failure domain; Argon2 and PostgreSQL share CPU; the global broadcast.
- **Upgrade trigger:** event-loop lag >50 ms sustained, or `wallets` row-lock waits
  climbing, or realtime egress → 100 Mbps.

### B) Recommended budget — two boxes (data isolated)
- **VPS1 8/8** — Node (3 roles) + Redis; **VPS2 8/8** — PostgreSQL (private network
  between them). Cloudflare Tunnel edge.
- **~RM 366 ≈ ₹7,000/mo.**
- **Capacity:** same 10k DAU, but the database no longer contends with Node for CPU, and
  an app restart doesn't touch it. Headroom for growth to ~15–20k.
...
- **VPS1 8/8** app; **VPS2 10/10** PostgreSQL (the database gets the headroom);
  **VPS3 4/4** Redis + workers; **+ edge 4/4** (or Cloudflare Tunnel to drop it).
...
| `wallets` row-lock wait p95 | < 50 ms; lock waits not climbing; no 40P01 deadlocks |
...
7. **PostgreSQL:** the only datastore, so it inherits both tiers' work — **start at
   4 vCPU / 8 GB / NVMe rather than the 2/2–4 the money-only tier used**, pool 10/proc
   (`pgClient.js`), **PgBouncer optional until the 3rd app node** (§6). Contention is
   `wallets` row locks, not a hot counter row — real pools are derived (§5). Exact size
   **UNKNOWN — load-test**; this is the number most changed by consolidation.
8. *(merged into 7 — there is one datastore.)*
9. **Redis:** ~256–512 MB; may co-locate with PostgreSQL at 10k; separate by ~25k (§7).
...
13. **Recommended architecture:** **B — two 8/8 VPS (~₹7,000/mo)**, database isolated.
...
16. **Can be colocated (at 10k):** Node roles together; PostgreSQL+Redis (+workers); edge via
