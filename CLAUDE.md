...
| **Every end-to-end workflow, as three actors, with what the platform actually answered** | `docs/reference/E2E_WORKFLOWS.md` — GENERATED by `npm run report:workflows`; never edited by hand |
...
| **Every control on every screen, and what happened when it was PRESSED** | `docs/reference/PANEL_CONTROL_COVERAGE.md` — GENERATED by `npm run report:controls` from the two artefacts a real browser leaves behind; never edited by hand |
