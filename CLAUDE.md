...
| Consecutive-rejection cap, and who may not serve whom | `SystemConfig.merchantOrderLimits.maxConsecutiveRejections` (schema default 3) for the cap; `order_rejections` for the pairs. A merchant who refuses an order is barred from that ORDER and from that PLAYER, applied in `assignmentCandidates`' WHERE. The streak is advanced and read in one `UPDATE … RETURNING`, and reset only by a COMPLETED order. See F-021. |
