...
| A player who does not pay | `domains/payment/playerPaymentFailure.service.js` — the ONE owner. `users.consecutive_payment_failures` advances on a buy that expires unpaid and is read in the statement that writes it; at `SystemConfig.merchantOrderLimits.maxConsecutivePlayerPaymentFailures` (schema default 5) the player is FLAGGED through the same `flagPaymentWarning` the merchant's red-flag button uses, so one player has one count however the failure was noticed. **Flagged, not blocked** — the answer to a pattern is a person. Cleared only by `moveDepositMoney`, where the money is known to have ARRIVED; clearing at PAID would let a false UTR wipe the record. |
