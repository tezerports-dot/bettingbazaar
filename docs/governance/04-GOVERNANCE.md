...
**2026-09-01 — "Top Winners" was ordered by recency, and nothing about the response said so.** `/api/v1/winners` merged its two sources — real settled bets and admin-curated entries — and sorted the result by `displayTime`. The nav item is called "Top Winners", the page is titled "Top Winners" with the subtitle "Biggest wins right now", and it renders the first three entries as a **podium**. So the podium showed the three most RECENT winners presented as the three biggest: a ₹50 win a minute ago outranked a ₹50,000 win an hour ago. The endpoint returned the right shape, the right count, and plausible names and amounts — it was wrong only about the one thing the feature is for, which is why the ordering is now a pure function in `winners.ranking.js` with a test, rather than a `.sort()` inside a route handler. Ties break on recency so the board cannot flicker between two requests over the same data, depending on which source was spread into the array first. **`sortOrder` selects, it does not rank:** it decides which curated entries make the shortlist when there are more than `limit` of them, and the amount then decides position — a pin that jumped the queue would put a chosen entry above a bigger real win on a board whose whole promise is "biggest wins".

**Three more defects on the same endpoint.** The real-wins query was capped at a hard-coded 20 while the curated one used `limit`, so the merge was silently biased toward curated entries whenever `limit` differed from 20. `page` was applied to the curated source ONLY, so page 2 returned the same real winners as page 1 — removed rather than fixed, because a top-N leaderboard does not page (the tail is not a second page of winners, it is not winners) and no caller ever sent it. And the `limit` clamp itself, written during this fix, had a bug the test caught before it shipped: `Number.isFinite` alone is not enough, because `Number(null)` and `Number('')` are both `0`, so a bare `?limit=` came out of `Math.max(1, …)` as a **one-entry leaderboard**. Anything not clearly asked for now takes the default.

**The query had no index and runs from a public unauthenticated endpoint.** `{status:'WON', isPhantom:false, settledAt:{$gte}}` sorted by `payout` was served by neither existing `bets` index — both lead with `cycleId`, which this query does not filter on — so it was a collection scan plus an in-memory sort over the largest collection in the system, getting slower every day the platform runs and 60× faster than that once 1-minute cycles are live. `winners_leaderboard` is ordered equality-then-sort (`payout: -1` third) so the index PROVIDES the order instead of feeding a blocking sort: an in-memory sort has a hard 32 MB ceiling and fails the request outright past it, where walking the index degrades into a longer scan. **Stated rather than implied:** an index bounds this cost, it does not make it small. At the load target a 24-hour window holds tens of millions of bets, and past that the leaderboard wants to be a maintained collection written at settlement rather than a query over `bets` — to be done when the window scan shows up in latency, not before.

**2026-09-01 — The cycle's REAL pools are derived from the bets, and the reason is a deadlock rather than a preference.** The obvious design — `real_delhi_paise` on the cycle row, incremented by every bet in the same transaction that places it — is the one thing this schema cannot have. A bet takes that row's SHARED boundary lock; incrementing a column on it is a lock UPGRADE, and two bets doing that concurrently deadlock: each holds a share lock the other must see released before it can take the exclusive one. **Demonstrated against PostgreSQL 16 rather than argued: two bets, one cycle, `40P01 deadlock detected`, one transaction killed.** The only way to keep the stored column is to make the bet's lock exclusive, which is the serialisation measured earlier this cycle at **~418 bets/sec against ~2,113 shared**, on one cycle, against a stated target of 500–800.

**Deriving is the stronger guarantee, not the consolation prize.** A total summed from `bets` cannot disagree with the bets; a stored counter can, and the Mongo document's six independently-`$inc`'d rupee floats are what that looks like in practice. `derivePoolsForCycle` is the single authoritative answer for both consumers that need one — the per-second realtime snapshot and the exact read the winner is determined from — served by `bets_cycle_pool_idx (cycle_id, side, status, stake_paise)` as an index-only scan that never touches a bet row. REFUNDED stakes are excluded because that money went back; WON and LOST are counted because they were staked.

**PHANTOM pools stay as columns on the cycle**, and the asymmetry is not arbitrary: phantom bets are never written to `bets` at all (`dualWrite` skips them — synthetic, no funding provenance, unsettleable), so there is nothing to derive them from; and the equalizer writes them ONCE per cycle rather than once per bet, so they carry none of the contention above. `phantom_bet_count` is included in the public `betCount` deliberately — a count that excluded phantom could be compared against the displayed pool to infer the split, which is the inference `assertPublicCycleSafe` exists to prevent.

**What this replaces from the earlier design in this same branch:** the `total_delhi_paise` / `total_bombay_paise` GENERATED columns are gone with the stored real pools they were generated from. The invariant they enforced — total = real + phantom — now holds because the total is composed at read time from a sum and a column, with no third copy to drift. The table is three commits old and exists only in development databases, so it was dropped and recreated rather than migrated.

**2026-09-01 — PostgreSQL owns the cycle, and the advisory lock is gone rather than kept.** Recorded as a correction to the two entries above, both of which reasoned from a constraint that no longer holds. Those entries say the lock must be ADVISORY because "there is no `cycles` table — the cycle lives in MongoDB". There is one now, built from scratch as the final design (the platform is pre-production; there was nothing to migrate). `betPg.placeBet` takes `SELECT … FOR SHARE` on the cycle row and `openSettlement` takes `FOR UPDATE` on the same row via a CTE, giving exactly the shared/exclusive semantics the advisory pair gave. **Two things improve, and one of them was a latent flaw in what shipped yesterday:** `hashtext` returns int32, so two unrelated cycle ids COULD collide and serialise against each other — never incorrect, since the lock stays mutual, but invisible and unreproducible when it happened; a row cannot collide, because the row is the identity. And the advisory lock locked a NUMBER, so the cycle's actual state needed a second query — which is why the CLOCK check could never live inside the bet's transaction and stayed in `bet.routes.js`, running *before* the transaction opened. `placeBet` now refuses three ways under the lock: `cycle_not_found` (a stake on a cycle that does not exist belonged to nothing, and was previously accepted), `cycle_expired` (read from `end_at` under the lock — the late-bet defect, closed at the write rather than ahead of it), and `cycle_settling`.

**The table is the final design, not a port of the document.** `isSettled` and the settlement totals are NOT duplicated onto it: `cycle_settlements` already owns the run, UNIQUE on `cycle_id`, and two copies of one state is precisely what the reverse mirror existed to keep in step — settlement state is a LEFT JOIN on a unique key. `isPaused` is gone (it was written alongside `status = 'PAUSED'` and said nothing more); `winnerDetermined` is gone (it said what `winner IS NOT NULL` says). Pools are BIGINT paise with `total_*` as **GENERATED** columns, so `total = real + phantom` is structural: the Mongo document carried six independently-`$inc`'d rupee floats and relied on every writer to keep them agreeing, and a test asserts the generated column cannot be written directly. Times are TIMESTAMPTZ in the table and epoch milliseconds in the returned objects, so the database gets a value it can index while callers keep the numbers they already read.

**One hard dependency, stated because it is an outage if it is ever silently broken:** the generator's `ensureCycle` call is awaited and allowed to throw. Without the row, every stake on that cycle is refused as `cycle_not_found` — so a swallowed failure there is a board that takes no bets, and learning that from a player is worse than learning it from a crash. **The test fixtures changed shape too, and that is the point rather than scaffolding:** the money suites used to bet against bare cycle-id strings, which was only possible because nothing in the transaction consulted the cycle. They now create the row first, which is the state the system can actually be in.
