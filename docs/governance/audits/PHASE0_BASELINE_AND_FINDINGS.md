...
authority for this repository, superseding `CLAUDE.md` where they conflict.
`CLAUDE.md` is NOT deleted — it is retained as the implementation-level ruleset
...
| "Legacy Token Rate logic no longer reflects the business model" (Phase 2.4, listed as REMOVE candidate) | **False — Token Rate is a live, intentional business mechanic**, not legacy | `CLAUDE.md` §1 names `TokenRates` model as the sole authority for buy/sell rates and states "Merchant earnings model: Buy/sell spread only. `Merchant.commissionRate` is retired." The spread *is* the commission model. Do not remove `TokenRates`. |
...
aggregate, `backend/routes/**` = domain API), not on a new tree. `CLAUDE.md` will be
...
### F4 — `CLAUDE.md` predates this audit's authority decision
