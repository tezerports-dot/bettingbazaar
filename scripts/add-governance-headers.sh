...
# GOVERNANCE sec.15: every file must reference CLAUDE.md in first 10 lines.
# Run: bash scripts/add-governance-headers.sh

HEADER='// GOVERNANCE: Read CLAUDE.md before editing this file. (See sec.0 for the mandatory pre-edit checklist.)'
ADDED=0
SKIPPED=0

while IFS= read -r file; do
  if head -10 "$file" | grep -q "GOVERNANCE: Read CLAUDE.md"; then
    SKIPPED=$((SKIPPED + 1))
    continue
  fi
