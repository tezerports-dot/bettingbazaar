 ...
 *
 * `reconciled` was in this set and has been REMOVED. It asked "do the two
 * stores agree?", which is the right question to ask before flipping authority
 * on a LIVE platform store by store — the flipped path keeps writing both, the
 * reconciler watches them, and disagreement is the signal to stop.
 *
 * There is no flip here. `authorityFor` returns POSTGRES unconditionally, so
 * every path already carries authority and no reconciler decides whether it
 * may. Worse, agreement is no longer evidence: Postgres writes the row and the
 * reverse mirror copies it into Mongo, so a cross-store comparison now reads a
 * value against a copy of itself and can only pass.
 *
 * The flag itself is kept as a DESCRIPTION of what exists per path — two paths
 * now honestly carry `reconciled: false` — but it is no longer a gate, because
 * the thing it gated does not happen.
 */
const REQUIRED_CAPABILITIES = Object.freeze(['implemented', 'dualWrite', 'rollback']);
